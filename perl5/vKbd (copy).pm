...
sub show_keyboard {
	...
	my $big_box = pEFL::Elm::Box->add($box);
	$big_box->size_hint_weight_set(EVAS_HINT_EXPAND, 0.3);
	$big_box->size_hint_align_set(EVAS_HINT_FILL,EVAS_HINT_FILL);
	$self->elm_parent()->pack_end($big_box);
	$self->elm_box($big_box);
	
	my $keyboard_box = pEFL::Elm::Box->add($big_box);
	$keyboard_box->size_hint_weight_set(EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
	$keyboard_box->size_hint_align_set(EVAS_HINT_FILL,EVAS_HINT_FILL);
	$big_box->pack_end($keyboard_box);
	$self->elm_kbd_box($keyboard_box);
	
	$self->_add_keys($layout);
	
	$self->is_visible(1);
	$keyboard_box->show(); $big_box->show();
}
...
sub _add_keys {
	...
	if ($l eq "###") {
		$keyboard_line_box = pEFL::Elm::Box->add($box);
		$keyboard_line_box->horizontal_set(1);
		$keyboard_line_box->size_hint_weight_set(EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
